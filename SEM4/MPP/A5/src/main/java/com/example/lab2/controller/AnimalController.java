...
public class AnimalController {
    ...
    public ResponseEntity<List<Animal>> filterAnimals(@PathVariable(value = "value") int numberLegs){
        List<Animal> animals = new ArrayList<Animal>();
        animals = animalService.filterAnimal(numberLegs);
        if (animals.isEmpty()) {
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        }
    ...
    @PostMapping("/shelters/{shelter_id}/animal")
    public ResponseEntity<Animal> addAnimal(@PathVariable(value = "shelter_id") Long shelterId,@Valid @RequestBody Animal animalRequest){
    ...
    public ResponseEntity<Animal> updateAnimal(@PathVariable("id") long id,@Valid @RequestBody Animal animalRequest){
    ...
    @PutMapping("/shelters/{shelter_id}/animalss")
    public ResponseEntity<List<Long>> addAnimal(@PathVariable(value = "shelter_id") Long shelterId,@Valid @RequestBody List<Long> animalRequest){
        Shelter s = shelterRepository.findById(shelterId).orElseThrow(() -> new ResourceNotFoundException("Animal " + shelterId + "not found"));
        for (Long i : animalRequest) {
            Animal a = null;
            a = animalRepository.findAnimalsById(i);
            if (a != null) {
                a.setShId(shelterId);
                a.setShelter(shelterRepository.findShelterById(shelterId));
